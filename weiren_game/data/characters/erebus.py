...
CHARACTER = CharacterDefinition(
    ...
    (A("draw_fate", "命运抽牌", "从命运牌堆中随机抽取3张不同的命运牌，并分别以50%的概率确定为正位或逆位。屋主从3张牌中选择1张，并立即结算其对应效果。牌面效果结算前，屋主可以选择给予厄瑞波斯一个紫色及以上品质的物品，决定卡牌的正位或逆位。牌面效果结算前，屋主可以选择反悔以取消本次选牌结果；反悔后，厄瑞波斯最大生命值上限将分别降低15/25/35。且在累计反悔3次后，本次对局内该能力失效。*每回合只能使用1次。", "fate", per_turn=True),),
)
