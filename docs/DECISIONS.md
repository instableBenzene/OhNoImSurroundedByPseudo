...
2. **合并靠"收集器"，不改各调用点**：`with self._log_sink() as sink: …` —— 块内的可见播报
   **不立刻播**，但仍逐条写进完整日志（`shown=False`）；退出时 `sink.flush(汇总文本)` 只播
   **一条**汇总（`text` = 汇总，`detail` = 被收集的逐条）。
   这样「数值一样就合并」不必动 `value_system` 的调用点，只在批量结算的外层包一层。
...
**验收**：一个回合的回合末结算，玩家日志只多**一行**（可点开看每人数值与原因）；
