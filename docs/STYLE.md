...
> 日志是玩家**唯一持续读到的正文**。它不需要文学性，需要**准、短、一致**。
> 机械项由 `tools/audit_text.py` 守着，别靠人记。

**句式**

- 主语是**角色名**，不是"你"；屋主不出现在播报里。
- 一条日志＝**一个事实**（谁 + 做了什么 + 结果）。**不解释、不评价、不猜**——"似乎 / 好像 / 也许 / 大概"一律不用。
- **长度上限 40 字**（现状平均 37）。超了就拆成两条，或删掉不承载信息的修饰。

**形态**

- **一律以 `。`（或 `！` `？`）收尾**。
- 数字用阿拉伯数字，单位跟在数字后（`理智 −5`、`持续 3 回合`）；负号全库统一用 `−`。
- 名字用 `“”` 引；**不用 `**` 加粗、不用 `·` 分隔、不换行**（换行只留给"回合开始/结束"这类结构行）。

**术语**

一个概念全库只用一个词；**照最近的用法，不自创同义词**（权威是代码里已经稳定的那批，不在这里另列一份清单）。

**风味 vs 播报**

- **播报**（机制）：必须能被复述成事实，上面的规则全部适用。
- **风味**（开场、天气、门外的感觉）：可以留白，用具体细节（声音 / 气味 / 时间 / 半句话）；
  不堆形容词，恐怖来自"不对劲"。语气见 `docs/PRINCIPLES.md` §一.8。

**写新日志前问三句**：主语清楚吗？后果说清了吗？还能更短吗？

**播报与完整日志分层**：玩家可见日志只放**汇总**（可点开看明细），逐条原文留给完整日志与「导出」。
同一批里数值相同的事件（回合末理智 / 生命自然流失等）**合并成一条**；机制与验收见 `docs/DECISIONS.md`
「日志分层」。操作回执（装备 / 卸下 / 整理携带位置这类玩家自己点的动作）默认只进完整日志。
