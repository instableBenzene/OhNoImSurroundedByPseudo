...
- 播报**默认无色**。只有"需要玩家立刻注意"的条目才带 `kind`（`_log(..., kind="warn")`）：
  `warn` = 损失预警 / 状态恶化；`danger` = 失败、误逐这类重后果。
- **少用**：一条日志的颜色价值来自它有多罕见——每屏都有颜色，等于没有颜色。
- 颜色只是**冗余**：文本本身必须写清后果（终端不保证颜色，也存在色觉差异）。
- 界面把 `kind` 映射到**锁定** token（`--warn` / `--danger`），所以换资源包也不会变色；
  内容层不写颜色，前端不写内容词。
- 现状：`warn` 两条（田里的作物**枯死**、罗兹**魔化**）；`danger` 四条（**死亡**、**驱逐**
  ——含替身被驱逐与"炼狱扳机"自我驱逐）。新增前先问："这条错过会亏吗？"——答不上来就别上色。

## 12. 图鉴 / 技能正文（排版）

> 这是**玩家会认真读**的另一块长文本（技能详情、图鉴条目）。已有的样板是 `data/codex_pack.py`
> 的伪人条目与 `data/codex_mechanics.py`（大量 `**` + 分行）——角色技能描述一直没跟上，
> 44 条里只有 1 条用加粗、0 条用换行，长的一坨 200+ 字。

**可用的标记**（渲染实现见 `webui/index.html::mdText`，**以那里为准**）：

| 写法 | 效果 |
| --- | --- |
| `·` | **行首要点**：每个 `·` 都转成换行（所以 `·` 只能当条目符号，**不能塞在句子中间**） |
| `\n` | 换行（同一段内的补充说明用它，别和 `·` 抢） |
| `**…**` | 加粗（数值、条件、代价） |
| `*…*` | 斜体淡色（脚注式备注，如"仅限…"） |
| `【…】` | 术语悬浮（**只在标签表里的词**会变成可悬浮，否则原样显示）；一段里 **≤2~3 个**，别满屏 |
| `“…”` | 对话：`“` 前自动换行 |

**规则**

1. **按长度分档**：一句话说清就单行；**>60 字按"机制 → 条件/数值 → 例外"分行**，
   每条一件事（长条目的读法靠分行，不靠标点）。
2. **数值与条件加粗**：`15%`、`最多 3 个`、`+50%`、`每回合 1 次`——照伪人图鉴的密度。
3. **限制类信息交给 `chips`，不写正文**：冷却 / 每回合次数 / 每局限一次 / 解锁条件
   都是 `AbilityDefinition.chips`（如 `chips=("冷却 3 回合",)`）；正文只讲**机制本身**。
   同一个限制**不在两处都写**。
4. **可量化进度不塞正文**：有数字的进度（暴露值、印记层数…）走 `mark_field` / `card_info`
   这类字段，界面按字段渲染；正文只说"怎么涨、涨到几"。
5. **改动前先看渲染**：正文最终由 `mdText` 排版，**写完要按真实渲染看一眼**（起 UI 或让探针过
   `fmt()`），别凭源码里的字符串想象版面。
6. **小标题与脚注各自另起一行**：`mdText` 只在 `·` 与 `\n` 处换行——"持有印记时：""*仅限…*"
   这类行如果直接接在上一句后面，**渲染时会被吞进同一行**（源码里看不出，必须看渲染）。
7. **引号不换行**：`“…”` 只用来框名字/术语/对话，**不会**自动断行（曾经会，结果是
   「入住时带来一把 / “燧发枪”和两枚 / “弹药-燧发枪”」这种把名字劈开的版面；
   全库 118 个引号里 113 个是名字）。要断行**显式写** `\n` 或 `·`。
8. **引号的分工**（全库已按此统一）：
   - `「…」` = **指称具体存在**：物资名 / 技能名 / 状态名 / 机制名（「田」「暴露值」「人设」）。
   - `“…”` = **对话与引语**（角色说的话、引述原稿的句子）。
   - `【…】` = **可悬浮术语**（只在标签表里的词才会变成可点的术语提示）。
   三者不要混用：同一个名字，全库只用同一种框法。
9. **乘号写 `×`，不要写 `*`**：`mdText` 里 `*` 是斜体标记（`×3` ✅ / `*3` ❌ 会被吃掉半个标记）。
   倍率、倍数一律 `×`（`×0.5`、`×1.5`）。
10. **中英混排加空格、标点后不留空格**：`ED Tear 入住时` ✅、`若 ED Tear 自身` ✅；
    `， 回合末` ❌（全角标点后又多一个空格）。
11. **物品描述照「薯条」那条的版式**：先一句"带来 / 可能获得什么"，再**单开一行**写
    `名字（品质）【标签】`，然后才是 `·` 要点，风味收进 `*…*` 脚注。
    同一条里出现两件物品时，每件都各自起一行。
12. **`·` 前面不要再写 `\n`**：`·` 本身就是换行，`\n·` 会撞出**空行**
    （渲染器已做兼容折叠，但内容里别写这种冗余）。
